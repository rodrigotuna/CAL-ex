...
bool Graph<T>::addBidirectionalEdge(const T &sourc, const T &dest, double w) {
    ...
    if (v1 == nullptr || v2 == nullptr)
        return false;
    Edge<T> *e1 = new Edge<T>(v1,v2,w);
    Edge<T> *e2 = new Edge<T>(v2,v1,w);
    e1->reverse = e2;
    e2->reverse = e1;
    v1->adj.push_back(e1);
    v2->adj.push_back(e2);
...
std::vector<Vertex<T>* > Graph<T>::calculatePrim() {
    ...
    while(!q.empty()){
        ...
        if(processed->visited) continue;
        processed->visited= true;
        for(auto e : processed->adj){
            if(e->dest->visited) continue;
...
std::vector<Vertex<T>*> Graph<T>::calculateKruskal() {
    ...
    std::sort(edgeList.begin(), edgeList.end(), [](Edge<T>* a, Edge<T>* b){
        return a->weight < b->weight;
    });
    double price = 0;
    for(auto &e: edgeList){
        if(findSet(e->orig) != findSet(e->dest)){
            linkSets(findSet(e->orig), findSet(e->dest));
            e->selected = true;
